  ...
  function createChart(options) {
    ...
    this.data.series.forEach(function(series, seriesIndex) {
      ...
      normalizedData[seriesIndex].forEach(function(value, valueIndex) {
        ...
        var text = '';
        if( options.stackBars ) {
          var prev = normalizedData[seriesIndex-1];
          if( prev ) text = '' + ( value + prev[valueIndex] );
        }
        else {
          text = '' + value;
        }
    ...
    for (var i = 0; i < this.data.series.length; i++) {
      ...
      if(options.showLabel) {
        ...
        var labelPosition = Chartist.polarToCartesian(center.x, center.y, labelRadius, startAngle + (endAngle - startAngle) / 2),
          interpolatedValue = options.labelInterpolationFnc(this.data.labels ? this.data.labels[i] : dataArray[i], i, this.data, totalDataSum);
